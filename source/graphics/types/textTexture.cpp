...
TextTexture::TextTexture(SDL_Texture*  texture,
  ...
  , Texture(texture, w, h, TextureTypes::Text) {
    ...
    for (int i = 0; i < mLetters.size(); i++) {
        ...
        ASSERT_WITH_MESSAGE(SDL_SetTextureBlendMode(character, SDL_BLENDMODE_BLEND) != 0, SDL_GetError())
        ASSERT_WITH_MESSAGE(SDL_SetTextureAlphaMod(character, 255) != 0, SDL_GetError())
